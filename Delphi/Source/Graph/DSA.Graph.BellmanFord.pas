...
type
  ...
  private type
    ...
    TCmp_T = TComparer<T>;
    TStack_Edge = TLinkedListStack<TEdge_T>;
...
type
  TSparseWeightedGraph_int = TSparseWeightedGraph<integer>;
  TBellmanFord_int = TBellmanFord<integer>;
  TReadGraphWeight_int = TReadGraphWeight<integer>;
...
constructor TBellmanFord<T>.Create(g: TWeightGraph_T; s: integer);
var
  bool, pass, i: integer;
  e: TEdge_T;
  cmp: TCmp_T;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        bool := cmp.Compare(__add_T(__distTo[e.VertexA], e.Weight), __distTo[e.VertexB]);
        if (__from[e.VertexA] <> nil) and ((__from[e.VertexB] = nil) or (bool < 0)) then
        begin
          __distTo[e.VertexB] := __add_T(__distTo[e.VertexA], e.Weight);
          __from[e.VertexB] := e;
        end;
      end;
    end;
...
destructor TBellmanFord<T>.Destroy;
begin
  inherited Destroy;
end;

function TBellmanFord<T>.HasPathTo(b: integer): boolean;
begin
  Assert((b >= 0) and (b < __g.Vertex));
  Result := __from[b] <> nil;
end;

function TBellmanFord<T>.NegativeCycle: boolean;
begin
  Result := __hasNegativeCycle;
end;

function TBellmanFord<T>.ShortestPath(b: integer): TList_Edge;
var
  stack: TStack_Edge;
  e: TEdge_T;
  ret: TList_Edge;
...
function TBellmanFord<T>.ShortestPathTo(b: integer): T;
begin
  Assert((b >= 0) and (b < __g.Vertex));
  Assert(not __hasNegativeCycle);
  Assert(HasPathTo(b));
...
procedure TBellmanFord<T>.ShowPath(b: integer);
var
  list: TList_Edge;
  i: integer;
begin
...
function TBellmanFord<T>.__add_T(a, b: T): T;
var
  v1, v2: Variant;
  ret: T;
begin
  v1 := TValue.From<T>(a).AsVariant;
  v2 := TValue.From<T>(b).AsVariant;
  TValue.FromVariant(v1 + v2).ExtractRawData(@ret);
  Result := ret;
end;

function TBellmanFord<T>.__detectNegativeCycle: boolean;
var
  i: integer;
  e: TEdge_T;
  cmp: TCmp_T;

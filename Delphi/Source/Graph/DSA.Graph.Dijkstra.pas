...
type
  ...
  TDijkstra_int = TDijkstra<integer>;
...
constructor TDijkstra<T>.Create(g: TWeightGraph_T; s: integer);
var
  ipq: TMinIndexHeap;
  v, w, bool: integer;
  e: TEdge_T;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        bool := cmp.Compare(__add_T(__distTo[v], e.Weight), __distTo[w]);
        if (__from[w] = nil) or (bool < 0) then
        begin
          __distTo[w] := __add_T(__distTo[v], e.Weight);
          __from[w] := e;
...
destructor TDijkstra<T>.Destroy;
begin
  inherited Destroy;
end;

function TDijkstra<T>.HasPathTo(b: integer): boolean;
begin
  Assert((b >= 0) and (b < __g.Vertex));
  Result := __marked[b];
end;

function TDijkstra<T>.ShortestPath(b: integer): TList_Edge;
var
  stack: TStack_Edge;
  e: TEdge_T;
  ret: TList_Edge;
...
function TDijkstra<T>.ShortestPathTo(b: integer): T;
begin
  Assert((b >= 0) and (b < __g.Vertex));
  Assert(HasPathTo(b));
  Result := __distTo[b];
...
procedure TDijkstra<T>.ShowPath(b: integer);
var
  list: TList_Edge;
  i: integer;
begin
...
function TDijkstra<T>.__add_T(a, b: T): T;
var
  v1, v2: Variant;
  ret: T;
begin

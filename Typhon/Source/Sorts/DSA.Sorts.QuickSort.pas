...
type
  ...
  generic TQuickSort<T> = class
  private
    type
    TArr_T = specialize TArray<T>;
    ICmp_T = specialize IDSA_Comparer<T>;
    TInsertionSort_T = specialize TInsertionSort<T>;
...
type
  ...
  TDelphiSort_int = specialize TDelphiSort<integer>;
...
begin
  ...
  begin
    ...
    targetArr := CopyArray(sourceArr);
    TestSort('QuickSort'#9#9, targetArr, @TQuickSort_int(nil).Sort);

    targetArr := CopyArray(sourceArr);
    TestSort('QuickSort2'#9#9, targetArr, @TQuickSort_int(nil).Sort2);

    targetArr := CopyArray(sourceArr);
    TestSort('QuickSort3Ways'#9#9, targetArr, @TQuickSort_int(nil).Sort3Ways);

    targetArr := CopyArray(sourceArr);
    TestSort('DelphiSort'#9#9, targetArr, @TDelphiSort_int(nil).Sort);
...
class procedure TQuickSort.Sort(var arr: TArr_T; cmp: ICmp_T);
begin
  __cmp := cmp;
  __sort(arr, 0, Length(arr) - 1);
end;

class procedure TQuickSort.Sort2(var arr: TArr_T; cmp: ICmp_T);
begin
  __cmp := cmp;
  __sort2(arr, 0, Length(arr) - 1);
end;

class procedure TQuickSort.Sort3Ways(var arr: TArr_T; cmp: ICmp_T);
begin
  __cmp := cmp;
  __sort3Ways(arr, 0, Length(arr) - 1);
end;

class function TQuickSort.__partition(var arr: TArr_T; l, r: integer): integer;
var
  v: T;
  j, i: integer;
begin
...
class function TQuickSort.__partition2(var arr: TArr_T; l, r: integer): integer;
var
  v: T;
  j, i: integer;
begin
...
class procedure TQuickSort.__sort(var arr: TArr_T; l, r: integer);
var
  p: integer;
begin
  if r - l <= 15 then
...
class procedure TQuickSort.__sort2(var arr: TArr_T; l, r: integer);
var
  p: integer;
begin
  if r - l <= 15 then
...
class procedure TQuickSort.__sort3Ways(var arr: TArr_T; l, r: integer);
var
  i, gt, lt: integer;
  v: T;
begin
...
class procedure TQuickSort.__swap(var a, b: T);
var
  tmp: T;
begin
  tmp := a;

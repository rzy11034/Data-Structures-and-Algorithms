...
begin
  ...
  begin
    ...
    targetArr := CopyArray(sourceArr);
    TestSort('MergeSort_BU'#9#9, targetArr, @TMergeSort_int(nil).Sort_BU);
...
class procedure TMergeSort.Sort(var arr: TArr_T; cmp: ICmp_T);
begin
  __cmp := cmp;
  __sort(arr, 0, Length(arr) - 1);
end;

class procedure TMergeSort.Sort_BU(var arr: TArr_T; cmp: ICmp_T);
var
  i, n, sz: integer;
begin
  n := Length(arr);
...
class procedure TMergeSort.__merge(var arr: TArr_T; l, mid, r: integer);
var
  aux: TArr_T;
  i, j, k: integer;
begin
...
class procedure TMergeSort.__sort(var arr: TArr_T; l, r: integer);
var
  mid: integer;
begin
  //if l >= r then

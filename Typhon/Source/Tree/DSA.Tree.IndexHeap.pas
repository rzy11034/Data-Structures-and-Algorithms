...
type
  ...
  var
    ...
    procedure __swap(var a, b: integer);
...
type
  TIndexHeap_T = specialize TIndexHeap<integer>;
...
begin
  ...
  with TIndexHeap_T.Create(n, THeapkind.Max) do
  begin
    for i := 0 to n - 1 do
  ...
  with TIndexHeap_T.Create(n, THeapkind.Min) do
  begin
    for i := 0 to n - 1 do
      Insert(i, Random(1000));
  ...
  with TIndexHeap_T.Create(sourceArr, THeapkind.Min) do
  begin
    SetLength(testArr, n);
    for i := 0 to n - 1 do
      testArr[i] := ExtractFirst;
  ...
  with TIndexHeap_T.Create(sourceArr, THeapkind.Max) do
  begin
    SetLength(testArr, n);
    for i := 0 to n - 1 do
      testArr[i] := ExtractFirst;
...
procedure TIndexHeap.Change(i: integer; e: T);
begin
  Assert(Contain(i));
...
function TIndexHeap.Contain(i: integer): boolean;
begin
  Assert((i + 1 >= 1) and (i + 1 <= __capacity));
  Result := __reverse[i + 1] <> 0;
end;

constructor TIndexHeap.Create(capacity: integer; cmp: ICmp_T; heapKind: THeapkind);
begin
...
constructor TIndexHeap.Create(capacity: integer; heapKind: THeapkind);
begin
  Self.Create(capacity, TCmp_T.Default, heapKind);
end;

constructor TIndexHeap.Create(const arr: TArr_T; heapKind: THeapkind);
begin
  Self.Create(arr, TCmp_T.Default, heapKind);
end;

constructor TIndexHeap.Create(const arr: TArr_T; cmp: ICmp_T; heapKind: THeapkind);
var
  i: integer;
begin
...
destructor TIndexHeap.Destroy;
begin
  inherited Destroy;
end;

function TIndexHeap.ExtractFirst: T;
var
  ret: T;
begin
  Assert(__count > 0);
...
function TIndexHeap.ExtractFirstIndex: integer;
var
  ret: integer;
begin
  Assert(__count > 0);
...
function TIndexHeap.Find(i: integer): T;
begin
  Assert(Contain(i));
  Result := __data[i + 1];
end;

function TIndexHeap.FindFirst: T;
begin
  if __count <= 0 then
...
function TIndexHeap.FindFirstIndex: integer;
begin
  Result := __index[1] - 1;
end;

procedure TIndexHeap.Insert(index: integer; e: T);
begin
...
function TIndexHeap.IsEmpty: boolean;
begin
  Result := __count = 0;
end;

function TIndexHeap.Size: integer;
begin
  Result := __count;
end;

procedure TIndexHeap.__shiftDown(k: integer);
var
  j: integer;
begin
  case __heapKind of
    THeapkind.Min:
      ...
      begin
        ...
        if (j + 1 <= __count) and
          (__cmp.Compare(__data[__index[j + 1]], __data[__index[j]]) < 0) then
          Inc(j);
...
procedure TIndexHeap.__shiftUp(k: integer);
begin
  case __heapKind of
    THeapkind.Max:
      while (k > 1) and (__cmp.Compare(__data[__index[k div 2]],
          __data[__index[k]]) < 0) do
      begin
...
procedure TIndexHeap.__swap(var a, b: integer);
var
  tmp: integer;
begin
  tmp := a;

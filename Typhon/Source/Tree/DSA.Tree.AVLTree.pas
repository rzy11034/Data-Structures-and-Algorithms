...
type
  ...
  generic TAVLTree<K, V, TKeyComparer> = class
  private
    type
  ...
  var
    __root: TNode;
    __size: integer;
    __cmp: specialize IDSA_Comparer<K>;
    ...
    //    / \
    //   T1 T2
    function __rightRotate(y: TNode): TNode;
    ...
    //        / \
    //       T3 T4
    function __leftRotate(y: TNode): TNode;
...
function TAVLTree.Contains(key: K): boolean;
begin
...
begin
  inherited;
  __cmp := TKeyComparer.Default;
end;

function TAVLTree.Get(key: K): TPtrV;
var
  node: TNode;
begin
...
function TAVLTree.GetSize: integer;
begin
  Result := __size;
end;

function TAVLTree.IsBalanced: boolean;
begin
  Result := __isBalanced(__root);
end;

function TAVLTree.IsBST: boolean;
var
  list: TArrayList_K;
  i: integer;
  bool: boolean;
begin
  ...
  for i := 1 to list.GetSize - 1 do
    if __cmp.Compare(list.Get(i - 1), list.Get(i)) > 0 then
      bool := False
    else
      bool := True;
...
function TAVLTree.Remove(key: K): TPtrV;
var
  node: TNode;
begin
  node := __getNode(__root, key);
...
procedure TAVLTree.Set_(key: K; newValue: V);
var
  node: TNode;
  Value: TValue;
  s: string;
...
function TAVLTree.KeySets: TArrayList_K;
var
  list: TArrayList_K;
begin
  list := TArrayList_K.Create;
...
function TAVLTree.__add(node: TNode; key: K; Value: V): TNode;
var
  bool: integer;
...
begin
  ...
  bool := __cmp.Compare(Key, node.Key);
  if bool < 0 then
  ...
  // 更新Height
  node.Height := 1 + Max(__getHeight(node.Left), __getHeight(node.Right));
...
function TAVLTree.__getBalanceFactor(node: TNode): integer;
begin
  if node = nil then
    Exit(0);
...
function TAVLTree.__getHeight(node: TNode): integer;
begin
  if node = nil then
  begin
    Result := 0;
...
function TAVLTree.__getNode(node: TNode; key: K): TNode;
var
  bool: integer;
begin
  if node = nil then
...
function TAVLTree.__minimum(node: TNode): TNode;
begin
...
procedure TAVLTree.__inOrder(node: TNode; list: TArrayList_K);
begin
  if node = nil then
    Exit;
...
function TAVLTree.__isBalanced(node: TNode): boolean;
var
  balanceFactor: integer;
begin
  if node = nil then
...
function TAVLTree.__leftRotate(y: TNode): TNode;
var
  x, T2: TNode;
begin
  x := y.Right;
...
function TAVLTree.__remove(node: TNode; key: K): TNode;
var
...
function TAVLTree.__rightRotate(y: TNode): TNode;
var
  x, T3: TNode;
begin
  x := y.Left;
...
constructor TAVLTree.TNode.Create(newkey: K; newValue: V);
begin
  Self.key := newkey;
  Self.Value := newValue;
  Left := nil;
